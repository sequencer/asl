...
let parse_file_to_json (filename : string) : Yojson.Safe.t =
  let ic = open_in filename in
  let lexbuf = Lexing.from_channel ic in
  lexbuf.lex_curr_p <- { lexbuf.lex_curr_p with pos_fname = filename };
  try
    let ast = Parser.spec Lexer.token lexbuf in
    let ast_with_stdlib = Asllib.Builder.with_stdlib ast in
    let typed_ast, _env = Typing.type_check_ast ast_with_stdlib in
    close_in ic;
...
let parse_string_to_json (input : string) : Yojson.Safe.t =
...
let () =
  if Array.length Sys.argv < 2 then (
    Printf.eprintf "Usage: %s <asl_file>\n" Sys.argv.(0);
    exit 1
  ) else (
    let filename = Sys.argv.(1) in
    try
      let json = parse_file_to_json filename in
