...
=== Integer Type <t_int>
The `asl.int` type is defined as `!asl.int<constraint>`, where `constraint` is a `ConstraintKindAttr` (see @constraint_kind). If it's constrained, it is `PendingConstrained` when parsing, and should be explicitly converted by `E_ATC` (see @e_atc_int) to get the final constrained type.

=== Bitvector Type <t_bits>
The `asl.bits` type is defined as `!asl.bits<width, bitfields>`, where `width` is an `IntegerAttr` and `bitfields` is an `ArrayAttr` of `BitFieldAttr` (see @bitfield). A width of `-2` indicates a parameterized type, `-1` indicates the width is not calculated yet. Both bitfield and width should be explicitly converted by `E_ATC` (see @e_atc_bits).
...
`asl.expr.atc.array` takes a `length`(`!asl.int`) operand for array size specification using `ArrayIndexAttr`.
